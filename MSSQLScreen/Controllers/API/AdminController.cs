...
namespace MSSQLScreen.Controllers.API
{
    public class AdminController : ApiController
    {
        ...
        [APIAuthorize(Roles = "SUPERADMIN")]
        [HttpDelete]
        [Route("api/admin/remove/{admin_id}")]
        public IHttpActionResult Remove(int admin_id)
        {
            var getadm = _context.AdminAccounts.SingleOrDefault(c => c.Id == admin_id);
            if (getadm == null)
                throw new HttpResponseException(HttpStatusCode.NotFound);
            else
            {
                _context.AdminAccounts.Remove(getadm);
                _context.SaveChanges();
                return Ok();
            }
        }
